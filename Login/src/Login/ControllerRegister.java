...
public class ControllerRegister implements Initializable {
    ...
    private void validateInputs() {
        if (!isMissingInput()) {
            System.out.println("no missing inputs");
            ...
            if (UsernameCheck && EmailCheck && PasswordCheck && DOBCheck) {
    ...
    private boolean checkEmail() {
        ...
        if (!emailInput.getText().isEmpty()) {
            ...
            if (patternEmail.matcher(emailInput.getText()).matches()) {
                ...
                errorsLabel.setText("Email must be of format xxxx@xxxx.xxx, for example Msl2420@email.vccs.edu");
    ...
    private boolean checkDOB() {
        boolean output = false;
        System.out.println("DOB checked");
        return output;
    }
