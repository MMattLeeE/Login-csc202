...
public class ControllerRegister implements Initializable {
    ...
    private void addUser() {
        ...
        User newUser = new User(firstName,lastName,ssn,dob,gender,username,password,email,phoneNumber,profilePhoto);
        UserDB.getUsersArrayList().add(newUser);

        try {
            UserIO.writeUsers(UserDB.getUsersArrayList());
        } catch (IOException e) {
            System.err.println("cant write new user to dat file");
        }
    ...
    private boolean checkEmail() {
        ...
        if (!emailInput.getText().isEmpty()) {
            //found a regex pattern that is RFC822 compliant
            ...
            if (patternEmail.matcher(emailInput.getText()).matches()) {
                ...
                errorsLabel.setText(errorsLabel.getText() + "\n" + "+Email must be of format xxxx@xxxx.xxx, for example Msl2420@email.vccs.edu.");
                errorsLabel.setVisible(true);
